...
export async function runDiscoveryPipeline(
  ...
  try {
    ...
    if (robots.isAllowed(canonicalBaseUrl)) queueSet.add(canonicalBaseUrl);
    else pagesSkippedRobots++;

    for (const url of sitemapResult.pageUrls) {
      if (queueSet.size >= maxPages) break;
      if (robots.isAllowed(url)) queueSet.add(url);
      else pagesSkippedRobots++;
    }
    ...
    for (const pageUrl of targetUrls) {
      ...
      if (crawlResult.renderedVia === 'playwright') playwrightCount++;
      ...
      if (crawlResult.crawlStatus === 'failed') {
        ...
        try {
          ...
          console.error('[CRAWLER] Failed to persist failed-page evidence:', persistError);
        }
        continue;
      }
      ...
      try {
        ...
        if (pageId) pagesCrawled++;
        else pagesFailed++;
      } catch (persistError) {
        pagesFailed++;
        console.error(
          `[CRAWLER] Persistence failed for ${crawlResult.url}:`,
          persistError
        );
      }
      ...
      if (progressError) {
        throw new Error(`Failed to persist crawl progress: ${progressError.message}`);
      }
    }
