...
interface GroundingChunk { web?: GroundingChunkWeb; }

export interface GeminiProviderOptions { apiKey?: string; primaryModel?: string; fallbackModels?: string[]; }
...
const analysisResponseSchema: Schema = {
  ...
  properties: {
    ...
    entitiesDetected: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: {
    ...
    }, required: ['name', 'entityType'] }, description: 'Brands or organization entities mentioned in the response.' },
...
export class GeminiProvider implements AIVisibilityProvider {
  ...
  private formatError(err: unknown): string {
    ...
    if (message.includes('NOT_FOUND') || message.includes('404') || message.includes('is not found')) return `Gemini Model Unavailable: ${message.trim()}`;
    return message;
  }
  ...
  async runGroundedQuery(promptText: string): Promise<GroundedQueryResult> {
    ...
    for (const model of modelsToTry) {
      try {
        response = await ai.models.generateContent({ model, contents: [{ role: 'user', parts: [{ text: promptText }] }], config: { tools: [{ googleSearch: {} }] } });
        break;
      } catch (err: unknown) {
  ...
  async analyzeResponse(promptText: string, rawText: string, citations: GroundingCitation[], targetDomainName: string): Promise<ScanAnalysisResult> {
    ...
    const ownCitationIndex = citations.findIndex((c) => { const d = c.sourceDomain.toLowerCase().replace(/^www\./, ''); return d === normalizedTarget || d.endsWith(`.${normalizedTarget}`); });
    ...
    for (const model of modelsToTry) {
      try {
        response = await ai.models.generateContent({ model, contents: [{ role: 'user', parts: [{ text: `You are an objective AI search visibility auditor. Analyze ONLY the provided AI response text and web citations below for query "${promptText}".\nTarget Domain: "${targetDomainName}"\n\nCRITICAL INSTRUCTIONS:\n- Analyze ONLY what is explicitly stated in the response text or citations.\n- Do NOT infer or invent a mention, sentiment, or citation not present in this text.\n\nRESPONSE TEXT:\n${rawText}\n\nREAL CITATIONS:\n${citationSummary || 'None'}` }] }], config: { responseMimeType: 'application/json', responseSchema: analysisResponseSchema } });
        break;
      } catch (err: unknown) { attemptedErrors.push(`[${model}]: ${this.formatError(err)}`); }
    }
    ...
    const entitiesDetected = Array.isArray(parsed.entitiesDetected) ? (parsed.entitiesDetected as Array<Record<string, string>>).map((e) => ({ name: e.name || 'Unknown', entityType: e.entityType || 'company', snippet: e.snippet, sentiment: e.sentiment })) : [];
    return { mentioned, mentionFrequency, sentiment, rankPosition, entitiesDetected, confidence: typeof parsed.confidence === 'number' ? parsed.confidence : 0.85, summary: typeof parsed.summary === 'string' ? parsed.summary : 'Query executed successfully.' };
  }
}
