...
// 2.5 Flash/Pro are no longer available to new API users. Use Google's
// supported stable alias and a lightweight compatibility fallback.
...
export const DEFAULT_PROVIDER_CONFIGS: Record<string, ProviderConfig> = {
  ...
  chatgpt: { slug: 'chatgpt', displayName: 'ChatGPT (OpenAI)', isActive: false, primaryModel: 'gpt-4o', fallbackModels: ['gpt-4o-mini'], capabilities: { searchGrounded: true, structuredOutput: true, supportedModels: ['gpt-4o', 'gpt-4o-mini'] } },
  claude: { slug: 'claude', displayName: 'Claude (Anthropic)', isActive: false, primaryModel: 'claude-3-5-sonnet', fallbackModels: ['claude-3-5-haiku'], capabilities: { searchGrounded: false, structuredOutput: true, supportedModels: ['claude-3-5-sonnet', 'claude-3-5-haiku'] } },
  perplexity: { slug: 'perplexity', displayName: 'Perplexity AI', isActive: false, primaryModel: 'sonar-pro', fallbackModels: ['sonar'], capabilities: { searchGrounded: true, structuredOutput: true, supportedModels: ['sonar-pro', 'sonar'] } },
};

export function getProvider(slug: string, options?: { apiKey?: string; primaryModel?: string; fallbackModels?: string[] }): AIVisibilityProvider {
  const normalizedSlug = slug.toLowerCase().trim();
  switch (normalizedSlug) {
    case 'gemini':
    case 'google-gemini':
      return new GeminiProvider({ apiKey: options?.apiKey, primaryModel: options?.primaryModel || GEMINI_PRIMARY_MODEL, fallbackModels: options?.fallbackModels || GEMINI_FALLBACK_MODELS });
    case 'chatgpt':
    case 'openai':
