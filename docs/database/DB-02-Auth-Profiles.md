...
> 
> *Future mitigation strategy*: If administrative user search by email is required in a future release, it must be implemented via a trusted Server Action using server-side credentials or a secure RPC function.
...
Standard RLS `UPDATE` policies restrict *which row* a user can update, but do *not* restrict *which columns* can be modified. Without column-level permissions, an attacker could send a payload updating `role = 'admin'` or `is_onboarded = true`.
