...
BEGIN
  ...
  SELECT COALESCE(AVG(day_count), 0) INTO v_scan_rate
  FROM (SELECT gs::date AS day, COUNT(s.id)::numeric AS day_count FROM generate_series(current_date - 6, current_date, interval '1 day') gs LEFT JOIN public.ai_scans s ON s.created_at::date = gs::date GROUP BY gs::date) d;
  SELECT COALESCE(AVG(day_count), 0) INTO v_user_rate
  FROM (SELECT gs::date AS day, COUNT(u.id)::numeric AS day_count FROM generate_series(current_date - 6, current_date, interval '1 day') gs LEFT JOIN public.users u ON u.created_at::date = gs::date GROUP BY gs::date) d;

  RETURN jsonb_build_object(
    'users_total', (SELECT COUNT(*) FROM public.users),
    'users_admin', (SELECT COUNT(*) FROM public.users WHERE role = 'admin'),
    'users_owner', (SELECT COUNT(*) FROM public.users WHERE role = 'owner'),
    'users_onboarded', (SELECT COUNT(*) FROM public.users WHERE is_onboarded),
    'users_active_7d', (SELECT COUNT(*) FROM public.users WHERE last_login_at >= now() - interval '7 days'),
    'projects_total', (SELECT COUNT(*) FROM public.projects WHERE deleted_at IS NULL),
    'scans_total', (SELECT COUNT(*) FROM public.ai_scans),
    'scans_completed', (SELECT COUNT(*) FROM public.ai_scans WHERE status = 'completed'),
    'scans_failed', (SELECT COUNT(*) FROM public.ai_scans WHERE status = 'failed'),
    'scans_running', (SELECT COUNT(*) FROM public.ai_scans WHERE status = 'running'),
    'scans_queued', (SELECT COUNT(*) FROM public.ai_scans WHERE status = 'queued'),
...
REVOKE ALL ON FUNCTION private.admin_platform_snapshot_impl() FROM PUBLIC, anon, authenticated;
GRANT USAGE ON SCHEMA private TO authenticated;
GRANT EXECUTE ON FUNCTION private.admin_platform_snapshot_impl() TO authenticated;
...
SET search_path = pg_catalog, public, auth
AS $$
  SELECT private.admin_platform_snapshot_impl();
$$;

REVOKE EXECUTE ON FUNCTION public.admin_platform_snapshot() FROM PUBLIC, anon;
