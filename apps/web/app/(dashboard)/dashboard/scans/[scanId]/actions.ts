...
export async function getScanDetailsData(scanId: string): Promise<ActionResult<ScanDetailsData>> {
  try {
    ...
    const { data: scanRow, error: scanErr } = await supabase.from('ai_scans')
      .select('id, project_id, provider_id, prompt_text, model_name, status, is_mentioned, summary_markdown, raw_response, error_message, started_at, completed_at, created_at')
    ...
    const durationSeconds = scanRow.completed_at
      ? Math.max(0, Math.round(((new Date(scanRow.completed_at).getTime()) - new Date(scanRow.started_at || scanRow.created_at).getTime()) / 1000))
      : null;
    ...
    const { data: citationsData, error: citationError } = await supabase.from('citations')
      .select('id, url, title, position, is_own_domain, competitor_id').eq('ai_scan_id', scanId).order('position', { ascending: true });
    ...
    const { data: mentionRows, error: mentionError } = await supabase.from('entity_mentions')
      .select('id, context_snippet, sentiment, tracked_entities(name, entity_type)').eq('ai_scan_id', scanId).order('created_at', { ascending: true });
    if (mentionError) return { success: false, error: mentionError.message };
    const mentionSummary = (mentionRows || []).map((m) => {
      const entity = Array.isArray(m.tracked_entities) ? m.tracked_entities[0] : m.tracked_entities;
      return { id: m.id, entityName: entity?.name || 'Unknown entity', entityType: entity?.entity_type || 'other', snippet: m.context_snippet, sentiment: m.sentiment || 'neutral' };
    });
    ...
    const platformBreakdown = providerRows.map((p) => {
      const scans = (providerScans || []).filter((s) => s.provider_id === p.id);
      const mentioned = scans.filter((s) => s.is_mentioned).length;
      return { provider: p.slug, displayName: p.display_name, isAvailable: p.is_active, score: scans.length ? Math.round((mentioned / scans.length) * 100) : null, summary: p.id === scanRow.provider_id ? scanRow.summary_markdown : null };
    });
    ...
    const tier1ScanCompetitors = confirmedList.map((c) => {
      ...
      const citedRows = citationList.filter((cit) => cit.competitor_id === c.id);
      return { id: c.id, competitorId: c.id, name: c.name, domainName: host, visibilityScore: citationList.length ? Math.round((citedRows.length / citationList.length) * 100) : 0, mentionCount: citedRows.length };
    });
    ...
    for (const page of pageList) {
      const meta = Array.isArray(page.page_metadata) ? page.page_metadata[0] : page.page_metadata;
      if (meta?.schema_json) schemaCount++;
      if (meta?.title?.trim() && meta?.meta_description?.trim()) metadataCount++;
    }

    const { data: latestCrawl } = await supabase.from('crawl_sessions').select('pages_skipped_robots').eq('project_id', scanRow.project_id).order('created_at', { ascending: false }).limit(1).maybeSingle();
