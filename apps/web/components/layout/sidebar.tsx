...
export function Sidebar({ isOpen = false, onClose, isAdmin = false, className, ...props }: SidebarProps) {
  ...
  // Strip custom non-DOM props so they never reach the <aside> element
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed top-0 bottom-0 left-0 z-50 flex w-[230px] flex-col border-r border-[#e2e4e9] bg-[#faf9f6] text-slate-900 transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 shrink-0 select-none',
      ...
      >
        ...
        <div className="flex h-13 items-center justify-between px-4 border-b border-[#e2e4e9] bg-white">
          <Link href="/dashboard" className="flex items-center gap-2 group" onClick={onClose}>
            ...
            <div className="flex flex-col">
              <span className="font-bold text-slate-950 text-xs tracking-tight leading-none">
                AI Visibility OS
              </span>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3 space-y-5">
          <div>
            <p className="px-2.5 text-[10px] font-mono font-bold uppercase tracking-widest text-slate-400 mb-1.5">
              Platform Modules
            </p>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={onClose}
                    className={cn(
                      'flex items-center space-x-2.5 rounded px-2.5 py-1.5 text-xs font-medium transition-all relative',
                      isActive
        ...
        {/* Footer Account Area */}
        <div className="p-2.5 border-t border-[#e2e4e9] bg-white">
          <LogoutButton className="w-full justify-center text-xs font-semibold text-slate-700 hover:text-slate-950 hover:bg-slate-100" />
        </div>
      </aside>
    </>
  );
